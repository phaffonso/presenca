...
#header
    ...
    for nome in nomes_ord:
        ...
        for dia in dias:
            key = (dia, nome)
            presenca = [False,False]
            if(key in pres):
                presenca = calcPresenca(pres[key])
            soma = soma + sum(presenca)
            print(presenca)
        fout.write('"' + nome + '",' + str(soma) + '\n')
